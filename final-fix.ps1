...
# 1. Replace "Leave Your Feedback" button with feedback form
$c = $c.Replace(
                ...
                <div class="max-w-lg mx-auto bg-brand-50 rounded-2xl p-6 border border-brand-100">
                    <form id="feedbackForm" onsubmit="submitFeedback(event)">
                        <div class="mb-4">
                            ...
                            <input type="text" id="fbName" required class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-brand-500 outline-none" placeholder="Your full name">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Your Role *</label>
                            <select id="fbRole" required class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-brand-500 outline-none bg-white">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex gap-2" id="ratingStars">
                                <button type="button" class="text-2xl text-gray-300 hover:text-yellow-400" onclick="setRating(1)">★</button>
                                <button type="button" class="text-2xl text-gray-300 hover:text-yellow-400" onclick="setRating(2)">★</button>
                                <button type="button" class="text-2xl text-gray-300 hover:text-yellow-400" onclick="setRating(3)">★</button>
                                <button type="button" class="text-2xl text-gray-300 hover:text-yellow-400" onclick="setRating(4)">★</button>
                                <button type="button" class="text-2xl text-gray-300 hover:text-yellow-400" onclick="setRating(5)">★</button>
                            </div>
                            <input type="hidden" id="fbRating" value="0">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Your Feedback *</label>
                            <textarea rows="4" id="fbMessage" required class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-brand-500 outline-none resize-none" placeholder="Tell us about your experience..."></textarea>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Email (optional)</label>
                            <input type="email" id="fbEmail" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-brand-500 outline-none" placeholder="your@email.com">
                        </div>
...
$anchor = '                            <div class="text-gray-400 text-xs">Kakoba Community</div>
                        </div>
                    </div>
                </div>
            </div>
...
$newReviews = '                            <div class="text-gray-400 text-xs">Kakoba Community</div>
                        </div>
                    </div>
                </div>
                <div class="bg-brand-50 rounded-2xl p-6 border border-brand-100 hover:shadow-lg transition-all">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div><div class="font-semibold text-gray-900 text-sm">Anna K.</div><div class="text-gray-400 text-xs">Bishop Stuart University</div></div>
                    </div>
                </div>
                <div class="bg-brand-50 rounded-2xl p-6 border border-brand-100 hover:shadow-lg transition-all">
                    <div class="flex gap-1 mb-3">
                    ...
                    <p class="text-gray-700 italic leading-relaxed mb-4">"Bought my charger and earphones here. Cheapest prices in Mbarara! The owner is so kind and always helps students."</p>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-brand-200 rounded-full flex items-center justify-center text-brand-700 font-bold text-sm">M</div>
                        <div><div class="font-semibold text-gray-900 text-sm">Mary B.</div><div class="text-gray-400 text-xs">Bishop Stuart Student</div></div>
                    </div>
                </div>
                <div class="bg-brand-50 rounded-2xl p-6 border border-brand-100 hover:shadow-lg transition-all">
                    <div class="flex gap-1 mb-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div><div class="font-semibold text-gray-900 text-sm">David W.</div><div class="text-gray-400 text-xs">Bishop Stuart Student</div></div>
                    </div>
                </div>
            </div>
            <div class="text-center mt-10 fade-in">'

$c = $c.Replace($anchor, $newReviews)

# 3. Add JS for feedback form at end
$jsCode = '
        function submitFeedback(e) {
            ...
            if (!name || !role || !message || rating === "0") { alert("Please fill all fields and select a rating."); return; }
            const msg = encodeURIComponent(`New feedback from ${name} (${role})! Rating: ${"★".repeat(rating)}${"☆".repeat(5-rating)} Message: ${message}`);
            document.getElementById("feedbackMsg").classList.remove("hidden");
            document.getElementById("feedbackForm").reset();
            document.getElementById("fbRating").value = "0";
            document.querySelectorAll("#ratingStars button").forEach(b => { b.classList.remove("text-yellow-400"); b.classList.add("text-gray-300"); });
...
Set-Content $f $c
Write-Host "Feedback form + reviews added!" -ForegroundColor Green
